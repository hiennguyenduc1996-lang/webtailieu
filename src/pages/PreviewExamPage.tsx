...
export default function PreviewExamPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (exam.subject === 'ENGLISH') {
        ...
        // Part 1: 18 MCQ (0-17)
        for (let i = 0; i < 18; i++) {
            if (studentAnswers[i] === exam.answers[i]) finalScore += 0.25;
        }
        // Part 2: 16 True/False (18-33)
        for (let i = 0; i < 4; i++) {
            let correctInGroup = 0;
            for (let j = 0; j < 4; j++) {
                if (studentAnswers[18 + i * 4 + j] === exam.answers[18 + i * 4 + j]) correctInGroup++;
            }
        ...
        for (let i = 0; i < 6; i++) {
            if (studentAnswers[34 + i] === exam.answers[34 + i]) finalScore += 0.25;
        }
        ...
        for (let i = 0; i < 4; i++) {
            let correctInGroup = 0;
            for (let j = 0; j < 4; j++) {
                if (studentAnswers[12 + i * 4 + j] === exam.answers[12 + i * 4 + j]) correctInGroup++;
            }
            if (correctInGroup === 1) finalScore += 0.1;
            else if (correctInGroup === 2) finalScore += 0.25;
            else if (correctInGroup === 3) finalScore += 0.5;
        ...
        for (let i = 0; i < 6; i++) {
            if (studentAnswers[28 + i] === exam.answers[28 + i]) finalScore += 0.5;
        }
    } else {
        // Default (for older exams)
         let correctCount = 0;
        for (let i = 0; i < exam.answers.length; i++) {
            if (studentAnswers[i] === exam.answers[i]) {
                correctCount++;
    ...
    // Calculate correctCount (raw, not weighted) for stats
    let correctCount = 0;
    for (let i = 0; i < exam.answers.length; i++) {
        if (studentAnswers[i] === exam.answers[i]) correctCount++;
    }
